...
import { ArrowLeft, User, Rocket, Globe, CreditCard, UserX, Camera, Upload } from "lucide-react"
...
export function SettingsModal({ isOpen, onClose }: SettingsModalProps) {
  ...
  useEffect(() => {
    if (isOpen) {
      ...
      if (savedUserData) {
        ...
        const savedAvatar = localStorage.getItem("betareader_avatar")
        if (savedAvatar) {
  ...
  const handleAvatarUpload = (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file) {
      ...
      // Aqui você salvaria a imagem no servidor/storage
      // Por enquanto, vamos simular salvando no localStorage
      const reader = new FileReader()
      reader.onload = (e) => {
        const base64 = e.target?.result as string
        localStorage.setItem("betareader_avatar", base64)
      }
        ...
        <div className="space-y-6">
          {activeTab === "profile" && (
            <>
              ...
              <div className="space-y-6">
                {/* Avatar Section */}
                <div className="flex flex-col items-center space-y-4 p-6 bg-card/50 rounded-xl border border-border/50">
                  <div className="relative">
                    ...
                    <button
                      ...
                      className="absolute -bottom-1 -right-1 w-8 h-8 bg-primary hover:bg-primary/80 rounded-full flex items-center justify-center shadow-lg transition-colors"
                      disabled={isLoading}
                    >
                  ...
                  <div className="text-center">
                    <button
                      onClick={() => document.getElementById("avatar-upload")?.click()}
                      className="inline-flex items-center space-x-2 px-4 py-2 bg-primary/10 hover:bg-primary/20 text-primary rounded-lg transition-colors disabled:opacity-50"
                      disabled={isLoading}
                    >
                      <Upload className="w-4 h-4" />
                      <span className="font-medium">Enviar foto de perfil</span>
                    </button>
                ...
                <div className="space-y-4">
                  ...
                  <div>
                    ...
                    <Input
                      ...
                      disabled={isLoading}
                    />
                    <p className="text-xs text-gray-300 mt-1">Opcional - usado para contato em caso de problemas</p>
                  </div>
                  ...
                  >
                    <div className="flex items-center justify-between mb-2">
                      <div className="flex items-center space-x-2">
                        ...
                        <h5 className="font-semibold text-white">Avaliador Nacional</h5>
                      </div>
                      {evaluatorMode === "nacional" && (
                  ...
                  >
                    <div className="flex items-center justify-between mb-2">
                      <div className="flex items-center space-x-2">
                        ...
                        <h5 className="font-semibold text-white">Avaliador Internacional</h5>
                      </div>
                      {evaluatorMode === "internacional" ? (
